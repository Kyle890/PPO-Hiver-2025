    ...
    public static void main(String[] args) {
        ...
        while(true){
            ...
            if(choix.equals("1")){
                ...
                try {
                    int itemChoisi = Integer.parseInt(choix);
                    if (itemChoisi >= 1 && itemChoisi <= plats.length) {
                        facture.add(plats[itemChoisi - 1]);
                    } else {
                        System.out.println("Veuillez choisir un chiffre entre 1 et " + plats.length);
                    }
                } catch (NumberFormatException e) {
                    System.out.println("Veuillez écrire un chiffre entre 1 et " + plats.length);
                }
            }
                ...
                try {
                    int itemRetirer = Integer.parseInt(choix);
                    if (itemRetirer >= 1 && itemRetirer <= facture.size()) {
                        facture.remove(itemRetirer);
                    } else {
                        System.out.println("Veuillez choisir un chiffre entre 1 et " + facture.size());
                    }
                } catch (NumberFormatException e) {
                    System.out.println("Veuillez écrire un chiffre entre 1 et " + facture.size());
                }
            }
